...
// LOCAL OVERRIDE (al_folio_core assets/js/theme.js)
// 원본은 system -> light -> dark -> system 3단계로 순환하기 때문에,
// OS 설정이 light인 사람은 "system(=light)" -> "light" 사이에서 화면이 전혀
// 바뀌지 않아 dark로 가려면 두 번 눌러야 했다.
// 지금 눈에 보이는 테마를 기준으로 light <-> dark 만 뒤집도록 바꿔서
// 어느 방향이든 한 번의 클릭으로 전환되게 한다.
// (대신 클릭만으로는 "system" 설정으로 돌아갈 수 없다.)
